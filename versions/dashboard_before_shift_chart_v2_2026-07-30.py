...
.fleet-card{height:96px!important;min-height:96px!important;padding:9px 12px;border:1px solid var(--line);border-radius:14px 14px 9px 9px;background:linear-gradient(145deg,rgba(17,34,44,.98),rgba(10,23,31,.94));position:relative;overflow:hidden;margin-bottom:4px}
...
[class*="st-key-metric_list_"] [role="radiogroup"] label{width:100%!important;min-width:0!important;min-height:44px;padding:6px 7px!important;margin:0!important;border:1px solid rgba(180,215,225,.09);border-radius:7px;background:linear-gradient(135deg,#10232c,#0d1d25);transition:all .15s ease}
...
[class*="st-key-metric_list_"] [role="radiogroup"] label:nth-child(n+4):nth-child(-n+6){background:linear-gradient(135deg,#10242c,#0b1b23)}
...
[class*="st-key-metric_list_"] [role="radiogroup"] p{width:100%!important;text-align:center!important;font:600 9px/1.25 'DM Sans'!important;color:#e2edf0!important;letter-spacing:.005em}
...
def terminal_operation_matrix(frame, visit_id):
    ...
    rows.append(
        ...
        f'<td class="row-total">{filter_link(grand_total)}</td></tr>'
    )
    return (
        '<div class="terminal-mix"><div class="terminal-mix-head">'
        '<b>Terminal operation profile</b><span>Full / empty · ISO length</span></div>'
        f'<table><thead><tr><th></th><th colspan="4">TCR · '
        f'{filter_link(tcr_total, matrix_terminal="TCR")}</th>'
        f'<th colspan="4">TC3 · {filter_link(tc3_total, matrix_terminal="TC3")}</th>'
        f'<th>{filter_link(grand_total)}</th></tr>'
...
def shift_performance_chart(frame, treated_mask):
    history = frame.loc[treated_mask & frame["Last Move DT"].notna(), ["Last Move DT"]].copy()
    if history.empty:
        return (
            '<div class="shift-chart"><div class="shift-chart-head"><b>Shift rhythm</b>'
            '<span>No treated timestamps</span></div></div>'
        )
    ...
    summary["GMPH"] = summary["Moves"] / active_hours
    max_moves = max(1.0, float(summary["Moves"].max()))
    max_gmph = max(1.0, float(summary["GMPH"].max()))
    chart_width, chart_height = 480, 68
    left, right, top, baseline = 12, 10, 5, 51
    plot_width = chart_width - left - right
    step = plot_width / max(1, len(summary))
    bar_width = max(8, step * 0.55)
    bars, points, labels = [], [], []
    for position, row in summary.reset_index(drop=True).iterrows():
        x_center = left + step * position + step / 2
        bar_height = 40 * float(row["Moves"]) / max_moves
        bar_x = x_center - bar_width / 2
        bar_y = baseline - bar_height
        is_latest = position == len(summary) - 1
        fill = "#38e8d1" if is_latest else "#397a91"
        opacity = "1" if is_latest else ".72"
        tooltip = (
            f'{row["Shift Start"]:%d %b %H:%M} · '
            f'{int(row["Moves"]):,} moves · {float(row["GMPH"]):.1f} GMPH'
        )
        bars.append(
            f'<rect x="{bar_x:.1f}" y="{bar_y:.1f}" width="{bar_width:.1f}" '
            f'height="{bar_height:.1f}" rx="2" fill="{fill}" opacity="{opacity}">'
            f"<title>{html.escape(tooltip)}</title></rect>"
        )
        line_y = baseline - 40 * float(row["GMPH"]) / max_gmph
        points.append(f"{x_center:.1f},{line_y:.1f}")
        shift_no = {7: "S1", 15: "S2", 23: "S3"}.get(row["Shift Start"].hour, "S")
        labels.append(
            f'<text x="{x_center:.1f}" y="64" text-anchor="middle" '
            f'fill="#718995" font-size="5.5">{row["Shift Start"]:%d}{shift_no}</text>'
        )
    avg_moves = float(summary["Moves"].mean())
    avg_gmph = float(summary["GMPH"].mean())
    latest = summary.iloc[-1]
    return (
        '<div class="shift-chart"><div class="shift-chart-head"><b>Shift performance</b>'
        '<span>Bars · treated moves &nbsp; / &nbsp; line · inferred GMPH</span></div>'
        f'<svg viewBox="0 0 {chart_width} {chart_height}" preserveAspectRatio="none">'
        f'<line x1="{left}" y1="{baseline}" x2="{chart_width-right}" y2="{baseline}" '
        'stroke="rgba(180,215,225,.12)" stroke-width="1"/>'
        + "".join(bars)
        + f'<polyline points="{" ".join(points)}" fill="none" stroke="#ffbd59" '
        'stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/>'
        + "".join(
            f'<circle cx="{point.split(",")[0]}" cy="{point.split(",")[1]}" r="2" fill="#ffbd59"/>'
            for point in points
        )
        + "".join(labels)
        + "</svg>"
        '<div class="shift-chart-foot">'
        f'<span class="current">Latest shift<b>{int(latest["Moves"]):,}</b></span>'
        f'<span>Average moves<b>{avg_moves:.0f}</b></span>'
        f'<span>Average GMPH<b>{avg_gmph:.1f}</b></span>'
        f'<span class="peak">Peak GMPH<b>{float(summary["GMPH"].max()):.1f}</b></span>'
        "</div></div>"
    )
...
""",
            ...
            metric_records = [
                ...
                ("Cumulative", "treated"),
                ("Still to treat", "remaining"),
            ]
            metric_labels = [
                f"📋 Programme · {fleet_programme:,}",
                f"✅ Treated · {fleet_treated:,} · {fleet_pct_label}",
                f"⏳ Remaining · {fleet_remaining:,}",
                f"⬇ Import · P {safe_int(f_import.sum()):,} · T {treated_import_total:,} · R {remain_import:,}",
                f"📦 Export full · P {safe_int((f_export & f_full).sum()):,} · T {treated_export_full_total:,} · R {remain_export_full:,}",
                f"▫ Export empty · P {safe_int((f_export & f_empty).sum()):,} · T {treated_export_empty_total:,} · R {remain_export_empty:,}",
                f"◷ Current shift · {safe_int(treated_in_shift.sum()):,}",
                f"Σ Total treated · {fleet_treated:,}",
                f"! Still to treat · {fleet_remaining:,}",
            ]
            metric_by_label = {
            ...
            st.markdown(
                shift_performance_chart(fleet_units, f_treated),
                unsafe_allow_html=True,
            )
            st.markdown(
                terminal_operation_matrix(fleet_units, fleet_id),
                unsafe_allow_html=True,
            )
...
<div class="ops-strip">
 ...
 <span class="rate">Vessel GMPH<b>{shift_gmph:.1f}</b></span>
 <span>STS productivity<b>{'Verified below' if not fleet_report.empty else 'Awaiting crane source'}</b></span>
</div>
{report_html}
""",
                    unsafe_allow_html=True,
