...
MOVE_EVENT_API_SAMPLE_FILE = Path(
    BOOT_CONFIG.get(
        "app",
        "move_event_api_sample_file",
        fallback="data/moveEvent_api_response.xml",
    ).strip()
)
...
if not MOVE_EVENT_API_SAMPLE_FILE.is_absolute():
    MOVE_EVENT_API_SAMPLE_FILE = APP_DIR / MOVE_EVENT_API_SAMPLE_FILE
...
[class*="st-key-shift_stop_"] button{min-height:25px!important;padding:2px 4px!important;border-color:rgba(255,189,89,.35)!important;background:rgba(255,189,89,.08)!important;color:#ffd07a!important;font-size:8px!important}
...
def load_api_config():
    ...
    settings = {
        ...
        "move_event_url": _api_setting(
            config, "api", "move_event_url", "NEVIS_MOVE_EVENT_API_URL"
        ),
...
MOVE_EVENT_COLUMNS = [
    "Time Completed", "Event Type", "Move Kind", "Unit Nbr", "Crane CHE",
    "Carrier Visit", "Fetch CHE Login Name", "Put CHE Login Name",
]


def _selected_xml_file_frame(path, selected_columns):
    """Stream selected fields from a large saved Apex XML response."""
    columns = []
    indexes = []
    records = []
    for _, element in ET.iterparse(path, events=("end",)):
        if element.tag == "columns":
            columns = [(child.text or "").strip() for child in element]
            indexes = [columns.index(name) for name in selected_columns]
        elif element.tag == "row" and columns:
            values = [(child.text or "").strip() for child in element]
            records.append(
                [values[index] if index < len(values) else "" for index in indexes]
            )
            element.clear()
    return pd.DataFrame.from_records(records, columns=selected_columns)


def _prepare_move_events(events):
    events = events.copy()
    for column in MOVE_EVENT_COLUMNS:
        if column not in events.columns:
            events[column] = ""
    events = events[
        events["Event Type"].isin(["UNIT_DISCH", "UNIT_LOAD"])
        & events["Crane CHE"].astype(str).str.fullmatch(r"STS\d+", na=False)
    ].copy()
    events["Completed DT"] = events["Time Completed"].map(parse_n4_date)
    events["Visit"] = events["Carrier Visit"].astype(str).str.strip()
    events["STS"] = events["Crane CHE"].astype(str).str.upper().str.strip()
    events["Conductor"] = np.where(
        events["Event Type"].eq("UNIT_DISCH"),
        events["Fetch CHE Login Name"],
        events["Put CHE Login Name"],
    )
    events["Conductor"] = (
        events["Conductor"].replace("", "Unknown").fillna("Unknown").astype(str)
    )
    return events.dropna(subset=["Completed DT"])[
        ["Completed DT", "Visit", "STS", "Conductor", "Event Type", "Unit Nbr"]
    ]


@st.cache_data(show_spinner=False)
def read_move_event_sample(path):
    return _prepare_move_events(_selected_xml_file_frame(path, MOVE_EVENT_COLUMNS))
...
@st.cache_data(show_spinner=False, ttl=300)
def read_live_move_events():
    settings = load_api_config()
    if not settings["move_event_url"]:
        return pd.DataFrame(columns=[
            "Completed DT", "Visit", "STS", "Conductor", "Event Type", "Unit Nbr"
        ])
    return _prepare_move_events(
        _request_api_frame(settings["move_event_url"], {}, settings)
    )
...
def shift_performance_chart(frame, treated_mask, visit_id):
    ...
    if visit_id == "__aggregate__":
        chart_visit_ids = set(
            pd.concat([frame["I/B Actual Visit"], frame["O/B Actual Visit"]])
            .dropna().astype(str)
        )
    else:
        chart_visit_ids = {str(visit_id)}
    crane_activity = move_events[
        move_events.get("Visit", pd.Series(dtype=str)).astype(str).isin(chart_visit_ids)
    ].copy() if not move_events.empty else pd.DataFrame()
    if not crane_activity.empty:
        crane_activity["Shift Start"] = crane_activity["Completed DT"].map(
            shift_start_for_timestamp
        )
        crane_activity = crane_activity[
            crane_activity["Shift Start"].isin(summary["Shift Start"])
        ]
    active_sts = sorted(crane_activity["STS"].dropna().unique()) if not crane_activity.empty else []
    figure = go.Figure()
    category_colors = {
        "Import": "#38e8d1",
        "Export full": "#62a9ff",
    ...
    chart_max = float(max(1, summary["Moves"].max()))
    figure.add_bar(
        x=summary["Axis"],
        y=np.full(len(summary), -chart_max * 0.11),
        base=np.full(len(summary), -chart_max * 0.44),
        marker={
            "color": "rgba(185,208,215,.01)",
            "line": {"color": "rgba(0,0,0,0)", "width": 0},
        },
        text=summary.apply(
            lambda row: (
                f'{int(row["Stop Count"])} '
    ...
    stacked_bar_height = summary[
        ["Import", "Export full", "Export empty"]
    ].sum(axis=1).astype(float)
    movement_marker_y = stacked_bar_height + chart_max * 0.20
    chart_ceiling = max(
        chart_max * 1.48,
        float(movement_marker_y.max()) * 1.10,
    )
    activity_marker = {
        "symbol": "square",
        "size": 28,
        "color": "rgba(7,17,24,.88)",
        "line": {"width": 0.7, "color": "rgba(56,232,209,.34)"},
    }
    figure.add_scatter(
        x=summary["Axis"],
        y=movement_marker_y,
        ...
        hovertemplate=(
            ...
            "<br><b>⚡ %{customdata[1]:,} treated movements</b>"
            "<br>↓ %{customdata[3]:,} import"
            "<br>■ %{customdata[4]:,} export full"
            "<br>□ %{customdata[5]:,} export empty"
            "<br>%{customdata[2]:.1f} GMPH · fixed 8h shift"
            "<br>%{customdata[6]:.1f}h movement activity span"
            "<br>⏸ %{customdata[7]:,} stops · %{customdata[8]:,} inactive min"
            "<br><b>Click to inspect these containers</b><extra></extra>"
        ),
    )
    figure.add_scatter(
        x=summary["Axis"],
        y=np.full(len(summary), -chart_max * 0.23),
        mode="text",
        text=summary["Axis"],
        textposition="middle center",
        textfont={"color": "#b9d0d7", "size": 8},
    ...
    sts_colors = {
        "STS1": "#ff6577", "STS2": "#a98cff", "STS3": "#38e8d1",
        "STS4": "#62a9ff", "STS5": "#ffbd59", "STS6": "#f58bd5",
    }
    for sts_index, sts_name in enumerate(active_sts):
        sts_rows = crane_activity[crane_activity["STS"].eq(sts_name)]
        conductors_by_shift = (
            sts_rows.groupby("Shift Start")["Conductor"]
            .agg(lambda values: " / ".join(sorted(set(map(str, values)))))
        )
        sts_labels = summary["Shift Start"].map(
            lambda shift: (
                f'<b>{sts_name}</b> · {conductors_by_shift.get(shift, "")}'
                if shift in conductors_by_shift.index else ""
            )
        )
        figure.add_scatter(
            x=summary["Axis"],
            y=np.full(len(summary), -chart_max * (0.88 + sts_index * 0.18)),
            mode="text",
            text=sts_labels,
            textposition="middle center",
            textfont={"color": sts_colors.get(sts_name, "#dceef2"), "size": 8},
            name=sts_name,
            showlegend=True,
            hoverinfo="skip",
            cliponaxis=False,
        )
    chart_floor = chart_max * (
        0.66 if not active_sts else 1.02 + (len(active_sts) - 1) * 0.18
    )
    figure.update_layout(
        height=(250 if not active_sts else 282 + len(active_sts) * 18),
        margin={"l": 8, "r": 8, "t": 54, "b": 4},
        ...
        yaxis={
            ...
            "showticklabels": False,
            "fixedrange": True,
            "range": [-chart_floor, chart_ceiling],
        },
    )
    return figure, summary.reset_index(drop=True)


def render_shift_stop_buttons(summary, visit_id, visit_ids, key_prefix, host=None):
    """Render a clickable Plotly strip aligned exactly with the shift bars."""
    if summary.empty:
        return False
    stop_strip = go.Figure()
    stop_strip.add_bar(
        x=summary["Axis"],
        y=np.ones(len(summary)),
        text=summary["Stop Count"].map(lambda value: f'⏸ {int(value)}'),
        textposition="inside",
        insidetextanchor="middle",
        textangle=0,
        constraintext="none",
    ...
    stop_strip.update_layout(
        ...
        clickmode="event+select",
        dragmode=False,
        xaxis={"showgrid": False, "showticklabels": False, "fixedrange": True},
        yaxis={
            "range": [0, 1.05], "showgrid": False, "showticklabels": False,
            "zeroline": False, "fixedrange": True,
        },
    )
    target = host if host is not None else st
    event = target.plotly_chart(
        stop_strip,
        width="stretch",
        config={"displayModeBar": False, "scrollZoom": False},
        key=f"shift_stop_{key_prefix}",
        on_select="rerun",
        selection_mode="points",
    )
    points = list(event.selection.points)
    if points:
        custom_data = points[0].get("customdata", [])
        if custom_data:
            clear_container_inspector(clear_metric_widgets=True)
            st.session_state.detail_visit_id = visit_id
            st.session_state.detail_visit_ids = list(visit_ids)
            st.session_state.detail_metric = (
                "stops|" + pd.Timestamp(custom_data[0]).isoformat()
            )
            st.rerun()
    return False
...
def select_aggregate_metric(widget_key, visit_ids, metric_by_label):
    selected_label = st.session_state.get(widget_key)
    selected_metric = metric_by_label.get(selected_label)
    if not selected_metric:
        return
    clear_container_inspector(clear_metric_widgets=False)
    st.session_state.detail_visit_id = "__aggregate__"
    st.session_state.detail_visit_ids = list(visit_ids)
    st.session_state.detail_metric = selected_metric
    st.session_state.metric_widget_epoch = (
        int(st.session_state.get("metric_widget_epoch", 0)) + 1
...
try:
    if MODE == "test":
        ...
        move_events = (
            read_move_event_sample(str(MOVE_EVENT_API_SAMPLE_FILE))
            if MOVE_EVENT_API_SAMPLE_FILE.exists() else pd.DataFrame()
        )
        ...
        move_events = read_move_event_sample(str(MOVE_EVENT_API_SAMPLE_FILE))
    else:
        units, visits = read_live_data()
        try:
            move_events = read_live_move_events()
        except Exception:
            move_events = (
                read_move_event_sample(str(MOVE_EVENT_API_SAMPLE_FILE))
                if MOVE_EVENT_API_SAMPLE_FILE.exists() else pd.DataFrame()
...
if scope == "Fleet":
    ...
    if len(working_visits) > 1:
        ...
        aggregate_shift_start, aggregate_shift_end = shift_window(aggregate_last)
        aggregate_in_current_shift = aggregate_units["Last Move DT"].between(
            aggregate_shift_start, aggregate_shift_end, inclusive="left"
        )
        summary_shift_moves = safe_int(
            (aggregate_treated & aggregate_in_current_shift).sum()
        )
        summary_shift_gmph = summary_shift_moves / 8.0
...
<div class="fleet-card">
 ...
 <div class="fleet-ops-mini">
  ...
  <span class="gmph">Current shift · {aggregate_shift_start:%H:%M}–{aggregate_shift_end:%H:%M}<b>{summary_shift_moves:,} moves · {summary_shift_gmph:.1f} GMPH / 8h</b></span>
...
""",
        ...
        aggregate_metric_labels = [
            f"📋 **PROGRAMME**  \nΣ **{summary_programme:,}** containers",
            f"✅ **TREATED**  \nΣ **{summary_treated:,}** · **{summary_completion:.1f}%**",
            f"⏳ **REMAINING**  \nΣ **{summary_remaining:,}** containers",
        ]
        aggregate_metric_by_label = {
            aggregate_metric_labels[0]: "programme",
            aggregate_metric_labels[1]: "treated",
            aggregate_metric_labels[2]: "remaining",
        }
        aggregate_metric_epoch = int(
            st.session_state.get("metric_widget_epoch", 0)
        )
        aggregate_metric_key = (
            f"metric_list_{aggregate_metric_epoch}___aggregate__"
        )
        aggregate_panel.radio(
            "Click a consolidated metric to inspect containers",
            aggregate_metric_labels,
            index=None,
            key=aggregate_metric_key,
            label_visibility="collapsed",
            on_change=select_aggregate_metric,
            args=(
                aggregate_metric_key,
            ...
            if aggregate_points:
                ...
                if aggregate_curve in [0, 1, 2, 3, 4] and aggregate_custom:
                    ...
                    if pd.notna(aggregate_selected_shift):
                        ...
                        metric_prefix = "stops" if aggregate_curve == 3 else "shift"
                ...
                if selected_points:
                    ...
                    if selected_curve in [0, 1, 2, 3, 4] and custom_data:
                        ...
                        if pd.notna(selected_shift):
                            ...
                            metric_prefix = "stops" if selected_curve == 3 else "shift"
                            query_metric = metric_prefix + "|" + pd.Timestamp(
                                selected_shift
                            ).isoformat()
                            st.session_state.detail_visit_id = query_visit
